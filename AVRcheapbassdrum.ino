...
// My (Simon juhl) contact: contact@simonjuhl.net
...
public:
  ...
  {
    // fix so that m_speed will not be zero
    if (m_speed <= 0) {m_speed = pot2 + 1;} 
    else {m_speed = pot2;} 
  }
...
int main(void)
{
  ...
  for (;;)
  {
    ...
    // changed looping trigger to rising edge trigger
    if (PINB & _BV(2))
    {
      if (!g_timer & drumrise)
      {
